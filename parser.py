...
def parse_if_statement(tokens):
    ...
    if tokens and tokens[0]["tag"] == "else":
        tokens = tokens[1:]
        else_block, tokens = parse_statement_block(tokens)

    return {"tag": "if", "branches": branches, "else": else_block}, tokens
...
def test_parse_if_statement():
    ...
    ast, _ = parse_if_statement(tokenize("if(1){print 1}elif(2){print 2}else{print 3}"))
    assert ast == {
        "tag": "if",
        "condition": {"tag": "number", "value": 1},
        "then": {"tag": "block", "statements": [{"tag": "print", "value": {"tag": "number", "value": 1}}]},
